...
uint64 SizeFixDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	int32 weapon_id = static_cast<int32>(constant);
	...
	for( const ryml::NodeRef& levelNode : node["Levels"] ){
		...
		for( const ryml::NodeRef& gradeNode : levelNode["Grades"] ){
			...
			if( !gradeExists ){
				...
				for( int32 i = 0; i < ARRAYLENGTH( grade->chances ); i++ ){
					grade->chances[i] = 0;
				}
			}
...
int32 status_damage(block_list *src,block_list *target,int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int32 flag, uint16 skill_id)
{
	...
	int32 sp = (int32)cap_value(dsp,INT_MIN,INT_MAX);
	int32 ap = (int32)cap_value(dap,INT_MIN,INT_MAX);
	...
	**/
	...
	if( sc && sc->getSCE(SC_KAIZEL) && !map_flag_gvg2(target->m) ) { // flag&8 = disable Kaizel
		int32 time = skill_get_time2(SL_KAIZEL,sc->getSCE(SC_KAIZEL)->val1);
...
int32 status_heal(block_list *bl,int64 hhp,int64 hsp, int64 hap, int32 flag)
{
	...
	int32 sp = (int32)cap_value(hsp,INT_MIN,INT_MAX);
	int32 ap = (int32)cap_value(hap,INT_MIN,INT_MAX);
	...
	if (flag&16 && mbl) { // Max HP setting from Summon Flora/marine Sphere
		...
		if (ud) { 
			...
			switch(ud->skill_id) {
				...
				{
					int32 homblvl = status_get_lv(mbl);
	...
	if (battle_config.pet_lv_rate && pd->master) {
		...
		if (lv != pd->pet.level || opt&SCO_FIRST) {
			...
			if (!(opt&SCO_FIRST)) // Lv Up animation
				clif_misceffect( *pd, NOTIFYEFFECT_BASE_LEVEL_UP );
			...
			status_calc_misc(pd, &pd->status, lv);
...
*/
static int32 status_get_hpbonus_equip(TBL_PC *sd) {
	int32 bonus = 0;
...
*/
static int32 status_get_spbonus_equip(TBL_PC *sd) {
	int32 bonus = 0;
...
*/
static int32 status_get_spbonus_item(block_list *bl) {
	int32 bonus = 0;
 ...
 * @return AP rate
 */
static int32 status_get_apbonus_equip(TBL_PC *sd) {
	int32 bonus = 0;
 ...
 * @return AP bonus
 */
static int32 status_get_apbonus_item(block_list *bl) {
	int32 bonus = 0;
...
static uint32 status_calc_maxap_pc( map_session_data& sd ){
	...
	dmax += status_get_apbonus( &sd, STATUS_BONUS_FIX );

	double equip_bonus = ( dmax * status_get_apbonus_equip( &sd ) / 100 );
	// TODO: using bMaxAP* for usable items may cause rounding issue
	double item_bonus = ( dmax * status_get_apbonus_item( &sd ) / 100 );

	dmax += equip_bonus + item_bonus;

	// Aegis accuracy
	dmax += static_cast<int64>( dmax * status_get_apbonus( &sd, STATUS_BONUS_RATE ) / 100 );
...
{
	static int32 calculating = 0; ///< Check for recursive call preemption. [Skotlex]
	...
	if (!sd->state.permanent_speed) {
		...
		int32 pSpeed = base_status->speed;
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			int32 wlv = sd->inventory_data[index]->weapon_level;
	...
	if (!sc->empty()){
		...
		for( const auto& it : *sc ){
			if( std::shared_ptr<s_status_change_db> scdb = status_db.find( it.first ); scdb != nullptr && scdb->script != nullptr ){
				run_script( scdb->script, 0, sd->id, 0 );
			}
		}
	}
...
int32 status_calc_mercenary_(s_mercenary_data *md, uint8 opt)
{
	...
	if (opt&SCO_FIRST) {
		...
		if (md->master)
			status->speed = status_get_speed(md->master);
	}
...
int32 status_calc_npc_(npc_data *nd, uint8 opt)
{
	...
	status_calc_misc(nd, status, nd->level);
 ...
 * @param bl: Object to change int32 [PC|MOB|HOM|MER|ELEM]
 * @param sc: Object's status change information
	...
	switch (battle_config.slaves_inherit_mode) {
		case 1: //Always aggressive
			if (!status_has_mode(&md.status,MD_AGGRESSIVE))
				sc_start4(nullptr, &md, SC_MODECHANGE, 100, 1, 0, MD_AGGRESSIVE, 0, 0);
			break;
		case 2: //Always passive
			if (status_has_mode(&md.status,MD_AGGRESSIVE))
				sc_start4(nullptr, &md, SC_MODECHANGE, 100, 1, 0, 0, MD_AGGRESSIVE, 0);
			break;
		case 4: // Overwrite with slave mode
			sc_start4(nullptr, &md, SC_MODECHANGE, 100, 1, MD_CANMOVE|MD_NORANDOMWALK|MD_CANATTACK, 0, 0, 0);
			break;
		default: //Copy master
...
t_tick status_get_sc_def(const block_list* src, const block_list* bl, sc_type type, int32 rate, t_tick tick, uint8 flag)
{
	...
	int32 sc_def = 0, tick_def = -1; // -1 = use sc_def
...
bool status_change_start(block_list* src, block_list* bl, sc_type type, int32 rate, int32 val1, int32 val2, int32 val3, int32 val4, t_tick duration, uint8 flag, int32 delay) {
	...
	if (sc == nullptr)
		return false; // Unable to receive status changes
	...
	if (sc->getSCE(SC_GRAVITYCONTROL))
		return false; // !TODO: Confirm what statuses/conditions (if not all) are blocked.
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	if (!scdb->endreturn.empty()) {
		...
		if (isRemoved) // Something was removed, don't give the status
			return true; // Return true so that sc_start can be checked as success
	}
...
static bool status_change_start_post_delay(block_list* src, block_list* bl, sc_type type, int32 val1, int32 val2, int32 val3, int32 val4, int32 tick, uint8 flag)
{
		...
		case SC_BLESSING:
			if (bl->type == BL_PC) {
				...
				if (sc->getSCE(SC_CURSE)) {
					...
					return true; // End Curse and do not give stat boost
				} else if (sc->getSCE(SC_STONE)) {
					status_change_end(bl, SC_STONE);
					return true; // End Stone and do not give stat boost
				}
			}
			if(sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_HIGH)
	...
	if (status_change_entry* sce = sc->getSCE(type); sce != nullptr) {
		switch( type ) {
			...
			case SC_ENDURE:
				if(sce->val4 && !val4)
					return true; // Don't let you override infinite endure.
				if(sce->val1 > val1)
					return true;
				break;
			case SC_JAILED:
	...
	switch(type)
	{
		...
		case SC_RUN:
		{
			...
			val3 = (int32)(currenttick & 0x00000000ffffffffLL);
			val4 = (int32)((currenttick & 0xffffffff00000000LL) >> 32);
			tick = INFINITE_TICK;
			break;
		}
		case SC_KAAHI:
		...
		case SC_SPHERE_5:
			if( !sd )
				return false;	// Should only work on players.
			val4 = tick / 1000;
			if( val4 < 1 )
				val4 = 1;
			tick_time = 1000; // [GodLesZ] tick time
		...
		case SC_DANCING:
			{
				...
				if(val4 && val4 != BCT_SELF && (dsd=map_id2sd(val4))) { // End status on partner as well
					...
					if(dsc) {
						...
						dsc->val2 = 0;
						dsc->val4 = BCT_SELF;
						status_change_end(dsd, SC_DANCING);
					}
				}
...
TIMER_FUNC(status_change_timer){
	...
	case SC_TOXIN:
		if (sce->val4 >= 0) { // Damage is every 10 seconds including 3%sp drain.
			if (sce->val3 == 1) { // Target
				...
				if (status->sp < status->max_sp)
					status_heal(bl, 0, (int32)status->max_sp * 1 / 100, 1);
			}
		}
		break;
	...
	case SC_DEATHHURT:
		if (sce->val4 >= 0) {
			if (status->hp < status->max_hp)
				status_heal(bl, (int32)status->max_hp * 1 / 100, 0, 1);
		}
		break;
	...
	case SC_CBC:
		if(--(sce->val4) >= 0) { // Drain hp/sp
			int32 hp=0;
			int32 sp = (status->max_sp * sce->val3) / 100;
	...
	case SC_C_MARKER:
		if( --(sce->val4) >= 0 ) {
			...
			if (!caster || caster->m != bl->m) //End the SC if caster isn't in same map
				break;
			sc_timer_next(1000 + tick);
	...
	case SC_TALISMAN_OF_PROTECTION:
		if(--(sce->val4) >= 0){
			...
			if( ssd == nullptr || status_isdead(*ssd) || ssd->m != bl->m || sd == nullptr || ssd->status.party_id != sd->status.party_id ){
...
static int32 status_natural_heal(block_list* bl, va_list args)
{
	...
	{ // Apply sitting regen bonus.
		...
		if(flag&(RGN_SHP)) { // Sitting HP regen
			rate = (int32)(natural_heal_diff_tick * (sregen->rate.hp / 100.));
			if (regen->state.overweight)
		...
		if(flag&(RGN_SSP)) { // Sitting SP regen
			rate = (int32)(natural_heal_diff_tick * (sregen->rate.sp / 100.));
			if (regen->state.overweight)
				rate /= 2; // Half as fast when overweight.
			sregen->tick.sp += rate;
	...
	if(flag&RGN_SP) {
		// Interval to next recovery tick
		rate = (int32)(battle_config.natural_healsp_interval / (regen->rate.sp/100. * multi));
